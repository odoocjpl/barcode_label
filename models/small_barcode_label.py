...
class small_barcode_template(models.Model):
    _name = 'small.barcode.template'
    ...
    def default_get(self, fields_list):
        res = super(small_barcode_template, self).default_get(fields_list)
        if self._context.get('wiz_id') and self._context.get('from_wizard'):
            for wiz in self.env['small.barcode.label'].browse(self._context.get('wiz_id')):
                prod_list = []
    ...
    field_lines = fields.One2many('small.barcode.template.line', 'barcode_label_id', string = "Fields")


class small_barcode_template_line(models.Model):
    _name = 'small.barcode.template.line'
    ...
    name = fields.Selection([('default_code', 'Internal Reference'),
                             ...
                             ('short_name', 'Short Name'),], string = "Name")
    barcode_label_id = fields.Many2one('small.barcode.template', string = "Barcode Label ID")


class small_barcode_label(models.TransientModel):
    _name = 'small.barcode.label'
    ...
    def default_get(self, fields_list):
        prod_list = []
        res = super(small_barcode_label, self).default_get(fields_list)
    ...
    def _get_report_paperformat_id(self):
        xml_id = self.env['ir.actions.report.xml'].search([('report_name', '=',
                                                        'barcode_label.product_barcode_report_template')])
    ...
    design_id = fields.Many2one('small.barcode.template', string = "Design")
    ...
    barcode_type = fields.Selection([('Codabar', 'Codabar'), ('Code11', 'Code11'),
                                    ...
                                    ('QR', 'QR')],
                                   string = 'Type', default = 'EAN13')
    field_lines = fields.One2many('small.barcode.label.line', 'barcode_label_id', string = "Fields")
    product_lines = fields.One2many('small.product.label.qty', 'barcode_label_id', string = "Product List")
    ...
    def action_call_report(self):
        ...
        datas = {
            'ids': self._ids,
            'model': 'small.barcode.label',
            'form': data
        }
        return  self.env['report'].get_action(self, 'barcode_label.product_barcode_report_template', data = datas)

    @api.multi
    def save_design_template(self):
        if not self.make_update_existing:
            view_id = self.env['ir.model.data'].get_object_reference('barcode_label',
                                                    'wizard_small_barcode_template_form_view')[1]
            ...
            return {
                ...
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'small.barcode.template',
                'type': 'ir.actions.act_window',
                'target': 'new',
                'view_id':view_id,
                'context' : ctx,
...
class small_barcode_label_line(models.TransientModel):
    _name = 'small.barcode.label.line'
    ...
    name = fields.Selection([('default_code', 'Internal Reference'),
                             ...
                             ('lst_price', 'Sale Price'),
                             ('short_name', 'Short Name')], string = "Name")
    barcode_label_id = fields.Many2one('small.barcode.label', string = "Barcode Label ID")


class small_barcode_label_line(models.TransientModel):
    _name = 'small.product.label.qty'
